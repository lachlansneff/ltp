...
templates specified in ima policy file"
	...
	if echo "$keycheck_line" | grep -q "*keyrings*"; then
		tst_brk TCONF "ima policy does not specify a list of keyrings to check"
	fi
...
but no specified keyrings"
	...
	do
		...
		if [ "$digest" != "$expected_digest" ]; then
			tst_res TFAIL "incorrect digest was found for the $(echo "$line" | \
				cut -d' ' -f5) keyring"
		fi
	done
...
a valid x509 certificate"
	...
	tst_res TINFO "adding a certificate to the \".ima\" keyring ($CERT_FILE)"
	...
	sudo evmctl import $CERT_FILE "$keyring_id" > /dev/null || \
		tst_brk TCONF "unable to import a certificate into the .ima keyring"

	grep -F ".ima" "$ASCII_MEASUREMENTS" | tail -n1 | cut -d' ' -f6 | \
		xxd -r -p > $TEST_FILE || \
		tst_brk TCONF "certificate not found in ascii_runtime_measurements log"
